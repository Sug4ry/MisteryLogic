...
5. 各要素（人物、イベント、アイテム、トリック、動機、証拠）について、該当する情報が「推発・推論に基づく曖昧なもの」「証言にすぎず確証がないもの」であれば、`uncertainty` フラグを `true` に設定してください。確実な事実であれば `false` にします。
6. 過去の状態と明らかな矛盾がある場合（例：すでに`死亡`となっているはずの人物が以降の章で生存して行動している、またはアリバイが成立しない場所の移動など）は、`warnings`リストに具体的な警告メッセージを追加してください。矛盾がない場合は空のリストにしてください。
