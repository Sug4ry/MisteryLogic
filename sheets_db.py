...
def get_gspread_client():
    """Streamlitのsecretsから認証情報を読み込み、gspreadクライアントをキャッシュして取得する"""
    try:
...
@st.cache_data(ttl=60)
def load_state_from_sheet(book_title: str = "デフォルトのミステリー", force_refresh: int = 0) -> MysteryState:
    """指定されたタイトルのMysteryStateをGoogle Sheetsから読み込む (ttlキャッシュ付き)"""
    try:
        sheet = get_google_sheet()
        _migrate_legacy_if_needed(sheet)
...
def save_state_to_sheet(state: MysteryState, book_title: str = "デフォルトのミステリー"):
    ...
    try:
        ...
        # 保存直後にキャッシュをクリアして最新データが見えるようにする
        get_all_books.clear()
        load_state_from_sheet.clear()
